...
# ------------------------------------------
    ...
    form_map = {
        ...
        "IMM5710 (워킹퍼밋)": "imm5710.pdf",
        "IMM5257 (방문비자)": "imm5257.pdf",
        "IMM1294 (학생비자)": "imm1294.pdf",
    ...
    if form_map[selected_form] is not None:
        ...
        if os.path.exists(file_path):
            with open(file_path, "rb") as f:
                file_bytes = f.read()
            tmpl_part = {"mime_type": "application/pdf", "data": file_bytes}
    ...
    if st.session_state.prep_result:
        ...
        if not sections:
            st.error("서식에서 분석할 항목을 찾지 못했습니다. (파일이 동적(XFA) PDF인지 확인하세요)")
        else:
            full_text_list = []
            for sec in sections:
